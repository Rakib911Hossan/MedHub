...
class _MedicineDetailsPageState extends State<MedicineDetailsPage> {
  ...
  Future<void> _fetchMedicineDetails() async {
    try {
      medicineSnapshot = await FirebaseFirestore.instance
  ...
  try {
    ...
    final medicineName = widget.medicine['name']?.toString() ?? 'Unknown Medicine';
    final company = widget.medicine['company']?.toString() ?? '';
    final price = (widget.medicine['price'] as num?)?.toDouble() ?? 0.0;
    final discountPrice = (widget.medicine['discount_price_each'] as num?)?.toDouble() ?? 0.0;
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      final newCartItem = {
        ...
        'total_discount_price': discountPrice * _quantity, // Calculate total discount price
      };

      if (cartDoc.exists) {
        final medicines = List<Map<String, dynamic>>.from(cartDoc['medicines'] ?? []);
        
        // Remove all previous versions of this medicine
        final updatedMedicines = medicines.where((item) => 
  ...
  Widget build(BuildContext context) {
    ...
    final totalDiscountPrice = (medicine['discount_price_each'] ?? 0.0) * _quantity;
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // 3. Price Display
            Text(
              'Price: \$${(medicine['price'] ?? 0.0).toStringAsFixed(2)}',
            ...
            // 3. Discount Price Display (if available)
              ...
              Text(
                'Discount Price: \$${(medicine['discount_price_each'] ?? 0.0).toStringAsFixed(2)}',
                style: const TextStyle(
                  fontSize: 20,
                  fontWeight: FontWeight.bold,
                  color: Colors.red,
            ...
            if (medicine['description'] != null) ...[
              ...
              Text(medicine['description'], style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),
            ],
            ...
            if (medicine['discount_amount'] != null) ...[
              Text(
                'Total Discount Price: \$${totalDiscountPrice.toStringAsFixed(2)}',
                style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 10),
            ],
